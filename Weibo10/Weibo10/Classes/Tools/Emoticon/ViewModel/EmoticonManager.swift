...
//        }
    ...
    func emoticonText(string: String, font: UIFont) -> NSAttributedString {
        ...
        while count > 0 {
            ...
            if let em = emoticonWithString(emStr) {
                
                // 3> 根据 em 建立一个图片属性文本
                let attrText = EmoticonAttachment(emoticon: em).imageText(font)
                
                // 4> 替换属性字符串中的内容
                strM.replaceCharactersInRange(range, withAttributedString: attrText)
            }
        }
